...
def hlsvd(data, nsv_sought, dwell_time):
    ...
    nsv_found, singular_values, frequencies, damping_factors, amplitudes, phases = r[
        0:6
    ]
...
def create_hlsvd_fids(result, npts, dwell, sum_results=False, convert=True):
    ...
    """
    if convert:
        result = convert_hlsvd_result(result, dwell)

    freqs, damps, areas, phase = result[2:6]

    fids = np.zeros(
        (len(freqs), npts), dtype=complex
    )  # Jia. np.complex is deprecated in Numpy 1.20
